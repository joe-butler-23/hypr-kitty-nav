...
pub struct TmuxSessionInfo {
    pub name: String,
    pub is_named: bool, // true if name != session_id (approximate check for "named" session)
    pub window_count: usize,
    pub pane_count: usize, // Total panes in current window
}
...
pub fn get_tmux_session_info(session: &str) -> Option<TmuxSessionInfo> {
    ...
    if parts.len() >= 3 {
        ...
        // Simple heuristic: if name starts with a digit, it might be auto-generated,
        // but user asked for "named session". 
        // Better check: usually auto-named sessions are just numbers like "0", "1".
        // Let's assume if it parses as integer, it's unnamed, unless user named it "123".
        // A common convention is that default sessions are integers.
